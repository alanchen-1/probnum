...
class ProbabilisticLinearSolver(
    ...
    def solve_iterator(
        ...
        ----------
        prior
            Prior belief about the quantities of interest :math:`(x, A, A^{-1}, b)`
    ...
    def solve(
        ...
        belief
            ...
            over the solution :math:`x`, the system matrix :math:`A`,
            its (pseudo-)inverse :math:`H=A^\dagger` and the right hand side :math:`b`.
        solver_state
...
class ProbabilisticKaczmarz(ProbabilisticLinearSolver):
    ...
    Probabilistic analogue of the (randomized) Kaczmarz method [1]_ [2]_,
    taking prior information about the solution and randomly choosing rows
    of the matrix :math:`A_i` and entries :math:`b_i` of the right-hand-side
    to obtain information about the solution.
    ...
    .. [1] Kaczmarz, Stefan, Angenäherte Auflösung von Systemen linearer Gleichungen,
        *Bulletin International de l'Académie Polonaise des Sciences et des Lettres.
        Classe des Sciences Mathématiques et Naturelles. Série A,
        Sciences Mathématiques*, 1937
    .. [2] Strohmer, Thomas; Vershynin, Roman, A randomized Kaczmarz algorithm for
        linear systems with exponential convergence,
...
class MatrixBasedPLS(ProbabilisticLinearSolver):
    ...
    inverse. The solver makes use of prior information and iteratively infers
    the matrix and its inverse by matrix-vector multiplication.
...
class SymMatrixBasedPLS(ProbabilisticLinearSolver):
    ...
    Probabilistic linear solver updating beliefs over the symmetric system
    matrix and its inverse. The solver makes use of prior information and
    iteratively infers the matrix and its inverse by matrix-vector multiplication.
