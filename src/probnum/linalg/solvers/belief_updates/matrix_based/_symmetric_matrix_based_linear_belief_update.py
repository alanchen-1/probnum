...
class SymmetricMatrixBasedLinearBeliefUpdate(LinearSolverBeliefUpdate):
    r"""Symmetric Gaussian belief update in a matrix-based inference
    framework assuming linear information.

    Updates the belief over the quantities of interest of a linear system
    :math:`Ax=b` given symmetric matrix-variate Gaussian beliefs with
    symmetric Kronecker covariance structure and linear observations.
    The belief update computes
    :math:`p(M \mid y) = \mathcal{N}(M; M_{i+1}, W_{i+1} \otimes_s W_{i+1})`,
    [1]_ [2]_ such that

    .. math ::
        \begin{align}
            M_{i+1} &= M_i + (y - M_i s) u^\top
            + u (y - M_i s)^\top - u s^\top(y - M_i s)u^\top,\\
    ...
    def _symmetric_matrix_based_update(
        ...
        if not isinstance(matrix.cov, linops.SymmetricKronecker):
            raise ValueError(
                "Covariance must have symmetric Kronecker structure, "
                f"but is '{type(matrix.cov).__name__}'."
            )
